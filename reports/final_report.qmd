...
Unfortunately, an f1 score of `r as.numeric(slice(select(table_data,'logreg'),6))` for `LogisticRegression` is quite low and unlikely to be particularly useful in the broader business sense. This analysis can be used as the basis to understand what target f1, precision or recall target scores should be set for further analysis.
