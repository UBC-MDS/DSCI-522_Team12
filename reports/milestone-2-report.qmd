...
![Figure 2. Class Imbalance in the Target Column](assets/class_imbalance.png)\
...
```{r, message=FALSE}
table_data <- read_csv("./assets/unique_counts.csv")|>subset( select = -c(1) )
...
|                              | max_features = 1000    |                                    |
+------------------------------+------------------------+------------------------------------+
| product                      | OneHotEncoder (Binary) | Two Products Included              |
|                              |                        |                                    |
| sub_product                  | OneHotEncoder          | Categorical Values                 |
|                              |                        |                                    |
| issue                        | OneHotEncoder          | Categorical Values                 |
|                              |                        |                                    |
| company_public_response      | OneHotEncoder          | Categorical Values                 |
|                              |                        |                                    |
| company                      | OneHotEncoder          | Categorical Values                 |
|                              |                        |                                    |
| company_response_to_consumer | OneHotEncoder          | Categorical Values                 |
+------------------------------+------------------------+------------------------------------+
...
kable(table_data, caption = "Table 2. Model Performance and Score.")
```
